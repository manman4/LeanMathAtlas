...
Cauchy-Schwarz 2D `(a²+b²)(c²+d²) ≥ (ac+bd)²`,
対称不等式 `a²+b²+c² ≥ ab+bc+ca`,
立方 AM-GM `a³+b³+c³ ≥ 3abc`,
CS 和形 `(a+b+c)² ≤ 3(a²+b²+c²)`
...
| `bench_hard_chain` | chain rule `sin(2x)` | ✓ chain_rule_deriv_tactics (明示的 .comp) |
| `bench_hard_irrational_sqrt2` | `Irrational √2` | ✓ exact? |
| `bench_hard_emod` | `a % b % c = a % c` (c∣b) | ✓ exact? |
| `bench_hard_card_filter` | `card {k < 2n \| k % 2 = 0} = n` | ✓ FINSET_CARD_TEMPLATES (bijection) |
