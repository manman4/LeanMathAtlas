...
|---------|------|---------|
| Unreleased | 2026-06-16 | feat(auto_prove): record failed proof attempts to local-only `.auto_prove_failures.jsonl` with goal / timeout / selected-tactic metadata (ignored by git); refine tactic routing by target shape instead of whole context, extend generic real-power inequality handling to `<` / `>`, and stop spraying induction templates onto generic exponent goals |
