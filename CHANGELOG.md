...
|---------|------|---------|
| Unreleased | 2026-06-21 | feat(auto_prove): add generic `Continuous` templates that reuse hypotheses via `hf.comp hg` and `hf.continuousAt`; add trig-inequality templates combining `sin_sq_add_cos_sq` with `sq_nonneg` for broad `sin`/`cos` inequality goals; add `LeanMathAtlas` examples `continuous_comp`, `sin_sq_le_one`, and `two_mul_sin_sq_le_two` that match these generalized proof patterns; keep changes goal-shape-based and low-memory-friendly |
| 0.3.19 | 2026-06-20 | feat(auto_prove): Phase 3 two-stage `have` search; goal-shape-specific BFS tactic sets; goal-aware `apply?` finisher selection; `Tendsto` / `HasDerivAt` templates; REPL startup diagnostics with stuck-process teardown; class-specific tactic priority for `trig` / `limit`; add `check_targets.py` (low-memory dry-run) and `analyze_failures.py` (failure log summarizer); strengthen search-suggestion verification with lightweight follow-up closers and prioritize `have` candidates by goal-shape relevance |
