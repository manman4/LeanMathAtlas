...
|---------|------|---------|
| 0.3.20 | 2026-06-21 | feat(auto_prove): add generic `Continuous` templates that reuse hypotheses via `hf.comp hg` and `hf.continuousAt`; add trig-inequality templates combining `sin_sq_add_cos_sq` with `sq_nonneg` for broad `sin`/`cos` inequality goals; add `LeanMathAtlas` examples `continuous_comp`, `sin_sq_le_one`, and `two_mul_sin_sq_le_two` that match these generalized proof patterns; keep changes goal-shape-based and low-memory-friendly |
